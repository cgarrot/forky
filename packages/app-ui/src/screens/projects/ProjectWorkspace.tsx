...
import { AppLayout, Button, SaveIndicator } from '@forky/ui'
...
import { guestJoinApi, setLastProjectId, useStore } from '@forky/state'
...
export function ProjectWorkspace({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const data = await guestJoinApi(shareToken)
        if (data?.projectId) {
  ...
  return (
    <AppLayout sidebar={<AppSidebar />} sidebarWidth={280}>
