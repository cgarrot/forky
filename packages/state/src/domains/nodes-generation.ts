...
import {
  cancelNodeGenerationApi,
  cascadeNodeGenerationApi,
  createNodeGenerationEventSource,
  generateTitleApi,
  startNodeGenerationApi,
} from '../api'
import { ApiError } from '../core/utils/errorUtils'
import { useSettings, useStore } from '../store'
import { readOrchestrationMetadata, writeOrchestrationMetadata } from '@forky/shared-core'
import type { Node, TodoItem } from '@forky/shared-core'
...
async function startGeneration(params: { nodeId: string; model: string }) {
  const payload = await startNodeGenerationApi({ nodeId: params.nodeId, model: params.model })
  if (!payload?.streamId) {
...
async function generateTitle(prompt: string, response: string): Promise<string | null> {
  try {
    const payload = await generateTitleApi({ prompt, response })
...
export function useGeneration(nodeId: string) {
  ...
  const cancel = useCallback(async () => {
    ...
    try {
      await cancelNodeGenerationApi(nodeId)
    } catch {
      return
    }
  ...
  const generateNode = useCallback(async () => {
    ...
    try {
      ...
      const source = createNodeGenerationEventSource(nodeId, streamId)
  ...
  useEffect(() => {
    const generateNodeOnce = async (id: string, options: { markDescendants: boolean }) => {
      ...
      await new Promise<void>((resolve, reject) => {
        const source = createNodeGenerationEventSource(id, streamId)
    ...
    const handleCascade = (event: CustomEvent<{ nodeId: string }>) => {
      void (async () => {
        ...
        const payload = await cascadeNodeGenerationApi(rootId)
        const affectedNodesRaw = Array.isArray(payload?.affectedNodes) ? payload.affectedNodes : null
