...
export class AuthService {
  ...
  private async resolveUsername(email: string, username: string | undefined): Promise<string> {
    const base = (username ?? email.split('@')[0] ?? 'user')
      .toLowerCase()
      .replace(/[^a-z0-9_]+/g, '')

    const normalized = (base.length >= 3 ? base : `user${base}`.slice(0, 30)).slice(0, 30)

    const candidates: string[] = [normalized]

    for (let i = 0; i < 5; i += 1) {
      const suffix = randomBytes(3).toString('hex')
      candidates.push(`${normalized.slice(0, Math.max(0, 30 - (suffix.length + 1)))}-${suffix}`)
    }

    for (const candidate of candidates) {
      const existing = await this.prisma.user.findFirst({ where: { username: candidate } })
      if (!existing) {
        return candidate
      }
    }
  ...
  async register(dto: RegisterDto): Promise<{ tokens: Tokens; user: SafeUser }> {
    ...
    const tokens = await this.issueTokens(user.id, user.email, user.username, user.avatar)
  ...
  private async issueTokens(
    ...
    const accessToken = await this.signAccessToken(userId, email, username, avatar)
