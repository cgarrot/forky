...
export class NodeGenerationService {
  ...
  async startGeneration(nodeId: string, input: GenerateNodeDto) {
    ...
    void this.runGeneration({ nodeId, streamId, modelId, temperature, maxTokens })
  ...
  async cancel(nodeId: string) {
    ...
    await this.prisma.node.update({ where: { id: nodeId }, data: { status: 'IDLE' } })
  ...
  async cascade(nodeId: string) {
    const node = await this.prisma.node.findFirst({ where: { id: nodeId, deletedAt: null }, select: { projectId: true } })
    if (!node) {
      throw new NotFoundException('Node not found')
    }

    const descendants = await this.getDescendants({ projectId: node.projectId, rootNodeId: nodeId })
    ...
    return {
      ...
      data: {
        startedAt: new Date().toISOString(),
        affectedNodes: descendants.map((id) => ({ nodeId: id, status: 'STALE' as NodeStatus })),
  ...
  private async runGeneration(params: {
    ...
    try {
      ...
      const systemPrompt = project?.systemPrompt ? String(project.systemPrompt) : undefined
      ...
      for await (const delta of result.textStream) {
        ...
        emitted += 1
        const progress = Math.min(0.99, emitted / Math.max(1, Math.ceil(maxTokens / 20)))
      ...
      const tokens =
        'usage' in result &&
        typeof (result as unknown as { usage?: { totalTokens?: unknown } }).usage?.totalTokens === 'number'
          ? ((result as unknown as { usage: { totalTokens: number } }).usage.totalTokens)
          : null

      const summary = await this.summarize({ modelId, content: full, abortController })
      ...
      this.streams.emit(streamId, { done: true, summary, tokens: typeof tokens === 'number' ? tokens : null })
      this.collaboration.emitToProject(projectId, 'node:streaming', {
        nodeId,
        done: true,
        summary,
  ...
  private async summarize(params: { modelId: string; content: string; abortController: AbortController }): Promise<string | null> {
    ...
    try {
      ...
      const result = streamText({
        model,
        system: "Tu es un expert en synthèse. Crée un résumé ultra-concis (1 phrase) en français.",
  ...
  private async buildContextMessages(nodeId: string): Promise<LLMMessage[]> {
    ...
    if (parentIds.length === 1) {
      messages.push(...(await this.buildParentContext(parentIds[0], node.projectId)))
    } else if (parentIds.length > 1) {
      for (let i = 0; i < parentIds.length; i += 1) {
        const parentId = parentIds[i]
        const parentNode = await this.prisma.node.findFirst({ where: { id: parentId, deletedAt: null } })
        ...
        messages.push({ role: 'user', content: `--- Contexte ${label} ---\n${contextText}` })
      }
    }
  ...
  private async buildParentContext(nodeId: string, projectId: string): Promise<LLMMessage[]> {
    const node = await this.prisma.node.findFirst({ where: { id: nodeId, deletedAt: null, projectId } })
    if (!node) return []

    const parents = await this.prisma.edge.findMany({ where: { projectId, targetId: nodeId }, select: { sourceId: true } })
    const parentIds = parents.map((p) => p.sourceId)

    const messages: LLMMessage[] = []

    if (parentIds.length === 1) {
      messages.push(...(await this.buildParentContext(parentIds[0], projectId)))
    } else if (parentIds.length > 1) {
      for (let i = 0; i < parentIds.length; i += 1) {
        const parentId = parentIds[i]
        const parentNode = await this.prisma.node.findFirst({ where: { id: parentId, deletedAt: null, projectId } })
  ...
  private async getDescendants(params: { projectId: string; rootNodeId: string }): Promise<string[]> {
