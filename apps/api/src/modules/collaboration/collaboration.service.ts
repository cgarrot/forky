...
const isEnvelope = (value: unknown): value is PubSubEnvelope => {
  ...
  if (value.excludeSocketId !== undefined && typeof value.excludeSocketId !== 'string') {
    return false
  }
...
export class CollaborationService implements OnModuleInit, OnModuleDestroy {
  ...
  emitToRoom(envelope: PubSubEnvelope) {
    ...
    this.publisher.publish(CHANNEL, JSON.stringify(envelope)).catch((error: unknown) => {
      const message = isRecord(error) && typeof error.message === 'string' ? error.message : 'unknown'
  ...
  private emitLocal(envelope: PubSubEnvelope) {
    ...
    if (envelope.excludeSocketId) {
      target.except(envelope.excludeSocketId).emit(envelope.event, envelope.payload)
      return
    }
