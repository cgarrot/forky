...
import { CreateNodeDto, DeleteNodeDto, JoinProjectDto, MoveCursorDto, UpdateNodeDto } from './dto/collaboration.dto'
...
const extractToken = (client: Socket): string | undefined => {
  ...
  const headers = isRecord(client.handshake.headers) ? client.handshake.headers : undefined

  const headerAuth = headers
    ? headers.authorization
    : undefined

  const raw = typeof authToken === 'string' ? authToken : typeof headerAuth === 'string' ? headerAuth : undefined
...
const getUser = (client: Socket): { id: string; username?: string; avatar?: string } => {
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleJoinProject(
    ...
    projectPresence.set(client.id, { userId: user.id, username: user.username, avatar: user.avatar })
  ...
  handlePing(@ConnectedSocket() client: Socket) {
    const projects = getJoinedProjectIds(client)
    const users = projects.flatMap((projectId) => Array.from(this.presenceByProject.get(projectId)?.values() ?? []))
