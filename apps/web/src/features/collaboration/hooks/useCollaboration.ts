...
import type { NodeMetadata, NodeStatus } from '@forky/shared';
...
type CollaborationHookReturn = {
  ...
  createNode: (params: { projectId: string; id?: string; prompt: string; position: { x: number; y: number }; parentIds?: string[] }) => void;
...
export const useCollaboration = ({
  ...
  useEffect(() => {
    ...
    const handleNodeDeleted = (data: unknown) => {
      ...
      dispatchProjectTouch(
        typeof deletedAtRaw === 'string' && deletedAtRaw.length > 0
          ? deletedAtRaw
          : undefined,
      );
    };
  ...
  }, [projectId, token, onCursorMove, onNodeUpdate, onNodeStreaming, onNodeCreated, onNodeDeleted, onUserJoin, onUserLeave, dispatchProjectTouch]);
  ...
  const createNode = useCallback(
    (params: { projectId: string; id?: string; prompt: string; position: { x: number; y: number }; parentIds?: string[] }) => {
      if (socketRef.current?.connected) {
