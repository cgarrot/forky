                          ...
                          try {
                            if (actionData.actionType === 'codebase_search') {
                              ...
                              const response = await fetch('/api/codebase/search', {
                                ...
                                body: JSON.stringify({ rootPath, query, maxResults: 10 }),
                              })

                              const payload = (await response.json().catch(() => null)) as
                                | { matches?: { filePath: string; lineNumber: number; line: string }[]; error?: string }
                                | null

                              if (!response.ok) {
                                throw new Error(payload?.error ?? 'codebase_search failed')
                              }
                            ...
                            if (actionData.actionType === 'codebase_index') {
                              ...
                              const response = await fetch('/api/codebase/index', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ rootPath, maxFiles }),
                              })

                              const payload = (await response.json().catch(() => null)) as
                                | { indexId?: string; rootPath?: string; fileCount?: number; error?: string }
                                | null

                              if (!response.ok) {
                                throw new Error(payload?.error ?? 'codebase_index failed')
                              }
